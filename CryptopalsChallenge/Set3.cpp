...
void Set3Challenge24() {
	...
	// test token generated with rand() seeded to 0
	srand(0);
	...
	// test token generated with rand() seeded to epoch
	srand(now);
	ByteVector token3 = ByteVector(token_length);
	token3.random();
	cout << "Test token 3 " << (ByteRandom::test_token(&token3, 100) ? "matches" : "does not match") << " bytes generated by MT19937 seeded with current time (match not expected)" << endl;
